...
namespace Big
{
		...
		m_Swapchain(g_GameVariables->m_Swapchain)
	{
		...
		m_iconfont = ImGui::GetIO().Fonts->AddFontFromMemoryTTF(const_cast<std::uint8_t*>(iconfont), sizeof(iconfont), 32.f, &font_cfg);
		m_nasafont = ImGui::GetIO().Fonts->AddFontFromMemoryTTF(const_cast<std::uint8_t*>(nasa_font), sizeof(nasa_font), 20.f, &font_cfg);
		m_titlefont = ImGui::GetIO().Fonts->AddFontFromMemoryTTF(const_cast<std::uint8_t*>(title_font), sizeof(title_font), 23.f, &font_cfg);
	...
	void D3DRenderer::WndProc(HWND hWnd, UINT msg, WPARAM wParam, LPARAM lParam)
	{
		...
		g_Settings.m_LockMouse = g_UiManager->m_Opened && g_MainScript->ImGUIMode;
