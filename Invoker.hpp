...
extern "C" void	spoof(void* context, void* function, void* ret);
...
namespace Big
{
	...
	public:
		...
		void* m_nativeReturn = Signature("FF E3").Scan().As<void*>();
		...
		void EndCall(std::uint64_t hash)
		{
			...
			if (auto newHash = GetNewHash(hash); newHash.has_value())
			{
				if (auto handler = GetHandler(*newHash))
				{
					__try
					{
						spoof(&m_Context, handler, m_nativeReturn);
						g_GameVariables->m_FixVector3(&m_Context);
					}
